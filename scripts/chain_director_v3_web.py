#!/usr/bin/env python3
"""chain_director_v3_web.py -- LAN web console for chain_director_v3.py (stdlib only).

Fork of chain_director_v2_web.py pointed at the resident-UNet chain director:
the driver defaults to chain_director_v3.py, so the form keeps only the fl2va
engines (text / first-last frame anchors) -- the ref2va material fields, the
ref-image-size selector and the --clear selector are gone, and ref2va uploads
are rejected with a pointer to the v2 CLI. It has its own port, data dir and
pidfile so both consoles can run side by side.

Runs on the GPU host next to ComfyUI and exposes a dashboard on 0.0.0.0:8190 by
default so other LAN machines can drive chain_director_v3.py without ssh:
  * mirrors every chain_director_v3.py CLI parameter as form fields
  * multipart uploads for first/last-frame anchors (v3 is fl2va only; ref2va
    stays a CLI feature of chain_director_v2.py)
...
Runtime / control:
  start (foreground):   ~/ComfyUI-Deploy/comfyenv/bin/python ~/MiniMax-H3-Deploy/scripts/chain_director_v3_web.py
  start (background):   ~/ComfyUI-Deploy/comfyenv/bin/python ~/MiniMax-H3-Deploy/scripts/chain_director_v3_web.py --start
  stop:                 ~/ComfyUI-Deploy/comfyenv/bin/python ~/MiniMax-H3-Deploy/scripts/chain_director_v3_web.py --stop
  status:               ~/ComfyUI-Deploy/comfyenv/bin/python ~/MiniMax-H3-Deploy/scripts/chain_director_v3_web.py --status
...
Guards / conventions (mirror the CLI docs):
  ...
  * busy_guard: queued jobs wait while ComfyUI /queue is executing something, or
    while an external driver is alive with ComfyUI down (it may be restarting)
  * the driver owns the service lifecycle: v3 is resident by default, so a
    finished job leaves ComfyUI up with the FSDP UNet loaded and the next job
    skips the cold start; cancel is TERM-grade and stops the service

Layout under the data dir (default ~/MiniMax-H3-Deploy/.h3web_v3):
  h3web_v3.pid              daemon pid
  h3web_v3.log              daemon stdout/stderr (only when --start)
...
def external_chain_pids(own_children):
    """Pids of external chain_director_v*.py processes (not spawned by us).
    ...
    """
    try:
        out = subprocess.run(["pgrep", "-af", "chain_director_v"],
    ...
    for ln in out.splitlines():
        ...
        if len(parts) != 2:
            continue
        pid_s, cmd = parts
        if not (re.search(r"chain_director_v\d+\.py", cmd) and not re.search(r"chain_director_v\d+_web", cmd)):
            continue
        try:
            pid = int(pid_s)
        except ValueError:
...
class Manager:
    def __init__(self, root, driver, comfy_base):
        ...
        self.data = os.path.join(root, ".h3web_v3")
    ...
    def blocking_busy(self):
        """Reasons a fresh run must wait (or empty = free).
        ...
        chain_director processes alone do NOT count - a resident v3 service keeps
        the FSDP shards in VRAM by design, so "process alive" != "generating".
        """
        reasons = []
...
def build_cfg(fields, files, manager, form_tag_hint=None, inherit_cfg=None):
    ...
    if engine == "ref":
        raise ValueError("本控制台驱动 chain_director_v3，只支持 fl2va（纯文本 / 首末帧图）；"
                         "ref2va 参考素材请改用 CLI：chain_director_v2.py --ref-image/-video/-audio")
...
<footer>内网部署 · 无鉴权 · 端口 8190 · 同时只串行跑一条链。驱动脚本：chain_director_v3.py（常驻 UNet；CLI 语义镜像）。</footer>
...
function clearForeignFiles(e){
  ...
  const allow=(e==="i2v"?["first_image","last_image"]:[]);
...
function fillForm(cfg){
  ...
  const map={segments:"segments",dur:"dur",width:"width",height:"height",steps:"steps",seed:"seed"};
...
# --------------------------------------------------------------------------
def main():
    ap = argparse.ArgumentParser(description="LAN web console for chain_director_v3.py")
    ap.add_argument("--host", default="0.0.0.0")
    ap.add_argument("--port", type=int, default=DEFAULT_PORT)
    ap.add_argument("--root", default=None,
                    help="deploy root (default ~/MiniMax-H3-Deploy); data lives under .h3web_v3/")
    ap.add_argument("--driver", default=None,
                    help="chain_director_v3.py path (default: same dir as this script)")
    ...
    driver = os.path.abspath(a.driver or os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                      "chain_director_v3.py"))
    data = os.path.join(root, ".h3web_v3")
    os.makedirs(data, exist_ok=True)
    pidfile = os.path.join(data, "h3web_v3.pid")
    logfile = os.path.join(root, "h3web_v3.log")
        ...
        for _ in range(50):
            try:
                ...
                try:
                    ...
                    pass
                return
        print("pid %d still alive after 10s; check h3web_v3.log" % pid)
        sys.exit(1)
